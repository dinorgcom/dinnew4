...
export async function GET(request: Request) {
  ...
  if (witness.invitationTokenExpiresAt && witness.invitationTokenExpiresAt.getTime() < Date.now()) {
    return new Response("Link expired", { status: 403 });
  }

  if (!witness.statementFileUrl) {
    return new Response("Not found", { status: 404 });
  }

  return serveBlob(witness.statementFileUrl);
}
