...
import { witnesses, cases, kycVerifications } from "@/db/schema";
import { WitnessVerifyPage } from "@/components/witness-verify-page";

type PageProps = {
  params: Promise<{ token: string }>;
};

export default async function WitnessVerificationPage({ params }: PageProps) {
  ...
  const caseRows = await db
    ...
    .from(cases)
    .where(eq(cases.id, witness.caseId))
    .limit(1);
  ...
  return (
    <WitnessVerifyPage
      ...
      statementFileUrl={witness.statementFileUrl}
      token={token}
    />
  );
}
